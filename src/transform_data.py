...
def create_dataframe(path_name:str) -> pd.DataFrame:
    ...
    logging.info(f"DataFrame created successfully with {len(df)} records.")
    return df
...
def rename_columns(df: pd.DataFrame, columns_rename:dict[str, str]) -> pd.DataFrame:
    df = df.rename(columns=columns_rename)
    logging.info(f"Columns renamed according to mapping: {columns_rename}")
    return df
